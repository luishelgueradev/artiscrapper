...
Plans:
- [ ] 01-01: Browser spike — verify Cloak Docker tag 0.3.31, confirm `pws=0` doesn't interstitial, verify `Browser.is_connected()` catches all Cloak death modes (kill pid, network drop, OOM), confirm ephemeral `new_context()` clears cookies between Google fetches. Capture 5-10 raw SERP HTML fixtures. Output: `SPIKE.md §Browser` + fixtures in `tests/fixtures/serp/`.
- [ ] 01-02: LLM router spike — `GET /models` against the live router, measure median TTFT, test `format=json` and (if supported) `format=<json_schema>`, run two back-to-back identical calls to confirm system-prompt KV-cache reuse, document per-client concurrency limit. Hand-label 30-50 candidates for the LLM regression set. Output: `SPIKE.md §LLM` + `tests/fixtures/llm/labelled.jsonl`.
- [ ] 01-03: Visit/extraction spike — drive top-10 AR catalog hosts headed (Mercadolibre is excluded per VISIT-08; targets include Mayorista Frog, Distribuidora Romero, Casa Susy, Falabella AR, romero-jugueteria, etc.), capture HTML, run an exploratory hand-rolled JSON-LD + OG extractor, classify each fixture as JSON-LD-sufficient vs needs-microdata. Document Falabella 403 rate under httpx + `DEFAULT_HEADERS`. Output: `SPIKE.md §Visit` + fixtures in `tests/fixtures/catalog/` + D12 decision (hand-roll vs extruct).
